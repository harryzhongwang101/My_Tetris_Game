...
abstract class TetroPiece extends Actor
{
    private final int blockId = 5;
    private Location[][] r = new Location[4][4];


    TetroPiece(Tetris tetris)
    {
        super();
        this.tetris = tetris;
    ...
    {
        if (isStarting) {
            this.display(gameGrid, new Location(getX(), getY()));
            isStarting = false;
            nb = 0;
    ...
    // Gets rotation ID for child class
    protected int getRotId() {
        return this.rotId;
    }

    // getters for block name, will get override in child classes
